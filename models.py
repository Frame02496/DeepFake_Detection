...
class Classifier:
    ...
    def load(self, path):
        # Handle both old (.h5) and new (.weights.h5) formats
        if os.path.exists(path):
            self.model.load_weights(path)
            
    def save(self, path):
        self.model.save('meso4_vid.h5')
        self.model.save_weights(path)

class Meso4(Classifier, Model):
    """Original Meso4 for images"""
...
class UnifiedMeso4(Classifier):
    """Unified model that can handle both images and videos"""
...
def train_image_model(data_dir, weights_path=None, epochs=10, batch_size=32):
...
if __name__ == "__main__":
    ...
    training_mode = 'video'  # Options: 'image', 'video'
    
    if training_mode == 'image':
        print("Training image-based model...")
        model, history = train_image_model(
            IMAGE_DATA_DIR,
            weights_path='./weights/Meso4_DF.h5',  # Load pretrained for training
            epochs=EPOCHS
        )
        model.save('meos4.weights.h5')
        ...
        model.save('meso4_vid.weights.h5')
